      ...
      if (type === 'payment') {
        ...
        if (paymentData.status === 'approved' || paymentData.status === 'success') {
          ...
          const mailOptions = {
            ...
            subject: `Pago aprobado para la demanda ${demandaData.id} de Necesito Esto!`,
            html: `
              <p style="font-size: 18px; color: #333; font-weight: bold; background-color: #f0f8ff; padding: 10px; border-radius: 5px;">
                Hola ${nombrePagador},
              </p>

              <p>📌 El pago para la demanda ${demandaData.id}: <strong>${demandaData.detalle}</strong> ha sido aprobado con éxito. Aquí están los detalles:</p>
